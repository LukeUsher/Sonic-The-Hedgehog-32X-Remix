...
Revision:	= 1	; change to 1 for JP1 revision	;Mercury REV01
...
CheckSumCheck:
	...
	if SkipCheckSumCheck=1	;Mercury Skip Check Sum Check
		bra.s	CheckSumOk
	...
	if SkipCheckSumCheck=0	;Mercury Skip Check Sum Check
...
Pal_Sonic:	incbin	"palette\(Mercury) Sonic (warm palette).bin"
Pal_GHZ:	incbin	"palette\(Mercury) GHZ (warm palette).bin"
	else
Pal_Sonic:	incbin	"palette\Sonic.bin"
Pal_GHZ:	incbin	"palette\Green Hill Zone.bin"
	endc	;end Warm Palette Mod

Pal_LZ:		incbin	"palette\Labyrinth Zone.bin"
Pal_LZWater:	incbin	"palette\Labyrinth Zone Underwater.bin"

	if WarmPaletteMod=1	;Mercury Warm Palette Mod
Pal_MZ:		incbin	"palette\(Mercury) MZ (warm palette).bin"
Pal_SLZ:	incbin	"palette\(Mercury) SLZ (warm palette).bin"
	else
...
GM_Title:				; XREF: GameModeArray
	...
	if TitleScreenPaletteFix=1	;Mercury Title Screen Palette Fix
		moveq	#palid_GHZ,d0	; load GHZ palette
		bsr.w	PalLoad1
	endc	;end Title Screen Palette Fix
	...
	if CustomDemoDelay=1	;Mercury Custom Demo Delay
		move.w	#CustomDemoDelayTime,(v_demolength).w
	else
		move.w	#$178,(v_demolength).w ; run title screen for $178 frames
	endc	;end Custom Demo Delay
		
	if EnablePSBMessage=1	;Mercury Enable Press Start Button Message
	...
	Tit_ClrObj2:
		move.l	d0,(a1)+
		dbf	d1,Tit_ClrObj2
	endc	;end Enable Press Start Button Message
...
Tit_EnterCheat:
	...
	if SimpleCheatEntry=1	;Mercury Simple Cheat Entry
		move.l	#$01010101,(f_levselcheat).w	; activate all cheats
		sfx	sfx_Continue	; play continue sound when code is entered
	else
		move.b	#1,(a0,d1.w)	; activate cheat
		move.b	#sfx_Ring,d0	; play ring sound when code is entered
		bsr.w	PlaySound_Special
	endc	;end Simple Cheat Entry
...
SignpostArtLoad:			; XREF: GM_Level
	...
	if GiantRingArtLoadsWithEndSign=1	;Mercury Giant Ring Art Loads With End Sign
...
GM_Special:				; XREF: GameModeArray
	...
	SS_ClrNemRam:
		...
		move.b	#id_SonicSpecial,(v_objspace).w ; load special stage Sonic object
...
; ===========================================================================
	...
	if (SpinDashDustActive|InstaShieldActive)=1	;Mercury Spin Dash Dust, Insta-Shield
		include "_incObj\(Mercury) Sonic Effects.asm"
...
SS_MapIndex:
	...
	if HUDCentiseconds=1	;Mercury HUD Centiseconds
		...
		include	"_maps\(Mercury) HUD (centiseconds).asm"
	
	if HUDInSpecialStage=1	;Mercury HUD in Special Stage
		include	"_maps\(Mercury) HUD SS (centiseconds).asm"
	endc	;end HUD in Special Stage
	...
	if HUDInSpecialStage=1	;Mercury HUD in Special Stage
		include	"_maps\(Mercury) HUD SS.asm"
	endc	;end HUD in Special Stage
...
; End of function AddPoints
	...
	if HUDInSpecialStage=1	;Mercury HUD In Special Stage
		include	"_inc\(Mercury) HUD_Update_SS.asm"
	endc	;end HUD In Special Stage
...
Art_Dust:	incbin	"artunc\(Mercury) SpinDash Dust.bin"
		even
...
Nem_Shield:	incbin	"artnem\(Mercury) Shield (solid).bin"
		even
	else
...
Nem_Goggle:	incbin	"artnem\Unused - Goggles.bin" ; unused goggles
	...
	if DynamicSpecialStageWalls=1	;Mercury Dynamic Special Stage Walls
		include	"_maps\(Mercury) SS Walls (dynamic).asm"
	else
...
; ---------------------------------------------------------------------------
	...
	if DynamicSpecialStageWalls=1	;Mercury Dynamic Special Stage Walls
Nem_SSWalls:	incbin	"artnem_u\(Mercury) Special Walls (dynamic).bin" ; special stage walls
		even
	else
...
Nem_Orbinaut:	incbin	"artnem\(Mercury) Enemy Orbinaut (SLZ).bin"
	else
...
SoundEffects_Common: ;Mercury Spin Dash SFX
...
Kos_Z80:	incbin	sound\z80_1.bin
		...
		even
Music81:	incbin	"sound\Mus81 - GHZ.bin"
		even
Music82:	incbin	"sound\Mus82 - LZ.bin"
		even
Music83:	incbin	"sound\Mus83 - MZ.bin"
		even
Music84:	incbin	"sound\Mus84 - SLZ.bin"
		even
Music85:	incbin	"sound\Mus85 - SYZ.bin"
		even
Music86:	incbin	"sound\Mus86 - SBZ.bin"
		even
Music87:	incbin	"sound\Mus87 - Invincibility.bin"
		even
Music88:	incbin	"sound\Mus88 - Extra Life.bin"
		even
Music89:	incbin	"sound\Mus89 - Special Stage.bin"
		even
Music8A:	incbin	"sound\Mus8A - Title Screen.bin"
		even
Music8B:	incbin	"sound\Mus8B - Ending.bin"
		even
Music8C:	incbin	"sound\Mus8C - Boss.bin"
		even
Music8D:	incbin	"sound\Mus8D - FZ.bin"
		even
Music8E:	incbin	"sound\Mus8E - Sonic Got Through.bin"
		even
Music8F:	incbin	"sound\Mus8F - Game Over.bin"
		even
Music90:	incbin	"sound\Mus90 - Continue Screen.bin"
		even
Music91:	incbin	"sound\Mus91 - Credits.bin"
		even
Music92:	incbin	"sound\Mus92 - Drowning.bin"
		even
Music93:	incbin	"sound\Mus93 - Get Emerald.bin"
		even
; ---------------------------------------------------------------------------
...
SoundD0:	incbin	"sound\SndD0 - Waterfall.bin"
	...
	if SpinDashSFXActive=1 ;Mercury Spin Dash SFX
SoundD1:	incbin	"sound\(Mercury) SndD1 - SpinDash.bin"
		even
	endc	;end Spin Dash SFX
...
; end of 'ROM'
	...
	if (PaddingActive=1)	;Mercury Padding Active
Padding:
	align	(PaddingMegs<<20), PaddingValue
	endc
	;end Padding Active
